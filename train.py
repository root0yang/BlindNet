...
def train(train_loader, net, optim, curr_epoch, writer, scheduler, max_iter, style_loader=None):
    ...
    for i, data in enumerate(train_loader):
        ...
        for di, ingredients in enumerate(zip(inputs, inputs_color, gts, aux_gts)):
            ...
            if args.jit_only:
                outputs = net([input, input_color], gts=gt, aux_gts=aux_gt, img_gt=img_gt, global_step=step)
            else:
                outputs = net(input, gts=gt, aux_gts=aux_gt, img_gt=img_gt)
            ...
            if args.use_ca:
                _cml = outputs[outputs_index]
                outputs_index += 1
                _ccl = outputs[outputs_index]
                outputs_index += 1
                total_loss = total_loss + (args.w1 * _cml) + (args.w2 * _ccl)

            if args.use_cwcl:
                _cwcl = outputs[outputs_index]
                outputs_index += 1
                total_loss = total_loss + (args.w3 * _cwcl)

            if args.use_sdcl:
                _sdcl = outputs[outputs_index]
                outputs_index += 1
                total_loss = total_loss + (args.w4 * _sdcl)
            ...
            if args.local_rank == 0:
                if i % 50 == 49:
                    ...
                    logging.info(msg)
                    if args.use_ca:
                        loss_msg = '[Covariance Alignment Loss : {}]'.format(_cml.item() + _ccl.item())
                        logging.info(loss_msg)
                    if args.use_cwcl:
                        loss_msg = '[Class Wise Contrastive loss : {}]'.format(_cwcl.item())
                        logging.info(loss_msg)
                    if args.use_sdcl:
                        loss_msg = '[Semantic Disentanglement loss : {}]'.format(_sdcl.item())
                        logging.info(loss_msg)
